...
enum TokenType { ERROR, OPERATOR, VALUE, EOLN, QUIT, HELP, EOFILE };
...
void popAndEval(dynamicArray* valueStack, dynamicArray2* operatorStack)
{

    char oper = operatorStack->getOPTop();
    operatorStack->opPop();

    int var2 = valueStack->getTop();
    valueStack->pop();

    int var1 = valueStack->getTop();
    valueStack->pop();

    int v3 = eval(oper, var1, var2);
    valueStack->push(v3);
...
void processExpression(Token inputToken, TokenReader* tr)
{
    ...
    dynamicArray* valueStack = new dynamicArray();
    dynamicArray2* operatorStack = new dynamicArray2();
    int result;
    ...
    if(operatorStack->isOPEmpty() == false) {
        operatorStack->resetOP();
    }
    ...
    while(inputToken.equalsType(EOLN) == false) {
        ...
        /* The expression contains an OPERATOR */
            ...
            if(inputToken.equalsOperator('(') == true) {
                operatorStack->opPush(inputToken.getOperator());
            }

            if(inputToken.equalsOperator('+') || inputToken.equalsOperator('-')) {
                while(operatorStack->isOPEmpty() == false &&
                    (operatorStack->getOPTop() == '+' || operatorStack->getOPTop() == '-' ||
                          operatorStack->getOPTop() == '*' || operatorStack->getOPTop() == '/')) {
            ...
            if(inputToken.equalsOperator(')')) {
                while(operatorStack->isOPEmpty() == false && operatorStack->getOPTop() != '(') {

                    popAndEval(valueStack, operatorStack);
                }

                if(operatorStack->isOPEmpty() == true) {
                    printf("Empty Stack\n");
                    printf("-999");
                } else {
                    operatorStack->opPop();
    ...
    if(operatorStack->isOPEmpty() == false) {
        printf("Error, Operator Stack should be empty \n");
    }
